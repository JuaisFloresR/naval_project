...
type SortField = keyof RowPart | null;
...
export function RowPartTable({ data, onAdd, onUpdate, onDelete, onImport }: RowPartTableProps) {
  ...
  const [editValues, setEditValues] = useState<RowPartFormData | null>(null);
  ...
  const handleSort = (field: keyof RowPart) => {
    if (sortField === field) {
  ...
  const getSortIcon = (field: keyof RowPart) => {
  ...
  const handleExport = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('RowPart Data');

    // Add headers
    worksheet.columns = [
      { header: 'Part ID', key: 'id', width: 15 },
    ...
    a.href = url;
    a.download = 'rowpart-data.xlsx';
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const renderEditableCell = (field: keyof RowPartFormData, value: number, rowId: string) => {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold text-gray-900">Part Measurements</h3>
          <p className="text-sm text-gray-600 mt-1">
            {data.length} row{data.length !== 1 ? 's' : ''} • Manage part measurement data
          </p>
        </div>
                    ...
                    {Array.from({ length: 11 }, (_, i) => (
                      <div key={i} className="space-y-2">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={editValues[`value${i + 1}` as keyof RowPartFormData]}
            ...
            {Array.from({ length: 11 }, (_, i) => (
              <div key={i} className="space-y-2 sm:space-y-3">
                ...
                <Input
                  ...
                  type="number"
                  step="0.01"
                  value={newRow[`value${i + 1}` as keyof RowPartFormData]}
